...
cd "$ROOT"
# Browser-facing vs container-facing RPC deliberately differ: /config hands rpcUrl
# to the BROWSER, which cannot resolve host.docker.internal.
export BSC_PUBLIC_RPC_URL="$RPC_HOST"
export BSC_WS_URL="ws://127.0.0.1:8545"
export BSC_RPC_URL="$RPC_DOCKER"
export BSC_SERVER_RPC_URL="$RPC_DOCKER"
export FYUZ_CONTRACT_ADDRESS="$FYUZ_ADDR"
export CHAIN_ID="$LOCAL_CHAIN_ID"
# Uploaded logos are fetched by the BROWSER, so this must be the host-reachable
# MinIO, not the container-internal S3_ENDPOINT. The production default
# (s3.fyuz.fun) does not resolve locally and shows up as ERR_NAME_NOT_RESOLVED.
export S3_PUBLIC_URL="http://localhost:9000/forgepad"
# Must be the FORK BLOCK: anvil proxies getLogs for pre-fork blocks upstream, where
# the public node rate-limits us. Starting here keeps every getLogs anvil-local.
export INDEXER_START_BLOCK="$FORK_BLOCK"
