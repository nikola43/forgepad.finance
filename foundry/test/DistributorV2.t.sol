...
contract DistributorV2Test is Test {
    ...
    function setUp() public {
        ...
        distributor = new DistributorV2(address(coordinator), subId, KEY_HASH, poster);
    ...
    function test_upkeep_revertsWhenNothingDue() public {
        vm.prank(keeper);
        vm.expectRevert(DistributorV2.UpkeepNotNeeded.selector);
        distributor.performUpkeep("");
    }
    ...
    function test_upkeep_onlyForwarderWhenConfigured() public {
        ...
        vm.prank(rando);
        vm.expectRevert(DistributorV2.NotForwarder.selector);
        distributor.performUpkeep("");
    ...
    function test_distributeBatchRejectsZero() public {
        fundPot(10 ether);
        uint256 rid = runRound(0);
        vm.expectRevert(DistributorV2.ZeroBatchSize.selector);
    ...
    function test_gasStarvedPayoutResumesInsteadOfFreezing() public {
        ...
        while (roundStatus(rid) == 1) {
            ...
            (bool ok,) = address(distributor).call{gas: 3_000_000}(
                abi.encodeWithSelector(DistributorV2.distribute.selector, rid)
            );
    ...
    function test_setAutomationConfigRejectsLowGasFloor() public {
        vm.expectRevert(DistributorV2.GasFloorTooLow.selector);
    ...
    function test_roundsOpenOnlyOnScheduledSlots() public {
        ...
        vm.expectRevert(DistributorV2.PeriodNotElapsed.selector);
        distributor.startRound();
        (bool needed,,) = checkUpkeep();
    ...
    function test_setScheduleRejectsZeroPeriod() public {
        vm.expectRevert(DistributorV2.ZeroPeriod.selector);
    ...
    function test_noPotMeansNoRound() public {
        ...
        (bool needed,,) = checkUpkeep();
        assertFalse(needed);
        vm.expectRevert(DistributorV2.NothingToDistribute.selector);
        distributor.startRound();
    }
    ...
    function test_distributeRefusesWhenNoUsersWerePosted() public {
        ...
        vm.expectRevert(DistributorV2.RandomnessPending.selector);
        distributor.distribute(rid);
        vm.prank(poster);
        vm.expectRevert(DistributorV2.RandomnessPending.selector);
    ...
    function test_emptyHolderListIsRejectedOutright() public {
        ...
        uint256 rid = distributor.roundId();
        vm.prank(poster);
        vm.expectRevert(DistributorV2.InvalidPackedLength.selector);
    ...
    function test_forceStartRoundIsAdminOnly() public {
        fundPot(10 ether);
        vm.prank(rando);
        vm.expectRevert(DistributorV2.NotPoster.selector);
    ...
    function test_setWindowStartBlockedDuringActiveRound() public {
        fundPot(10 ether);
        performUpkeep(A_START);
        vm.expectRevert(DistributorV2.RoundAlreadyActive.selector);
    ...
    function test_postSharesRejectsRepost() public {
        ...
        vm.prank(poster);
        vm.expectRevert(DistributorV2.SharesAlreadyPosted.selector);
        distributor.postShares(rid, packedShares3());
    }
    ...
    function test_postSharesRejectsOversizedSet() public {
        ...
        uint256 rid = distributor.roundId();
        vm.prank(poster);
        vm.expectRevert(DistributorV2.TooManyHolders.selector);
        distributor.postShares(rid, packed);
    }
    ...
    function test_postSharesRejectsOverAllocation() public {
        ...
        uint256 rid = distributor.roundId();
        vm.prank(poster);
        vm.expectRevert(DistributorV2.SharesExceedTotal.selector);
        distributor.postShares(rid, packed);
    }
    ...
    function test_emergencyWithdrawLeavesClaimableBehind() public {
        ...
        vm.expectRevert(DistributorV2.ClaimTransferFailed.selector);
        distributor.claim();
    }
    ...
    function test_setPercentsRejectsOver100() public {
        vm.expectRevert(DistributorV2.BpsTooHigh.selector);
