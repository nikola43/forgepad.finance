...
contract GriefSyncReproTest is Test {
    ...
    function test_SyncDonationDoesNotBrickGraduation() public {
        address t = fyuz.createToken{value: 0.001 ether}("Grief", "GRF", 0, 0, 0, 1, block.timestamp);
        ...
        for (uint256 i = 0; i < 3000 && !_launched(t); i++) {
            vm.prank(buyer);
            fyuz.swapExactETHForTokens{value: 0.1 ether}(t, 0.1 ether, 0, block.timestamp);
        }
        ...
        uint256 fdv = (fyuz.getETHPriceByUSD() * TOTAL_SUPPLY * wethRes) / tokRes / 1e18;
        console.log("post-grief V2 FDV USD:", fdv / 1e18);
        // Opens at or above target (never materially below).
        assertGe(fdv, (fyuz.TARGET_MARKET_CAP_USD() * 97) / 100, "opened below target (LP arb leak)");
    }
