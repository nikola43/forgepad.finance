...
/// @title Fyuz leaderboard fee Distributor V2 (BSC mainnet)
...
///
/// @dev What changed vs V1 (src/Distributor.sol):
///
///   1. Chainlink Automation (v2.1 registry, AutomationCompatibleInterface)
///      drives the WHOLE round, not just the payout. checkUpkeep reports which
///      single action is due and performUpkeep executes it:
...
///
///   2. The round window is derived ON-CHAIN (windowStart -> block.timestamp)
///      instead of being passed in by the poster, because Automation has no
///      opinion about calendar time. windowStart only advances on a PAID round,
///      so a cancelled round can never silently swallow a window's holders —
///      the same rule the backend's epoch_start() uses.
///
///   3. distribute() is GAS-AWARE and RESUMABLE. V1 pushed to all 100 holders in
///      one call, which costs ~1.4M gas for a normal holder set but ~6.6M if
///      every receiver burns its stipend and fails — above what an Automation
///      upkeep can be given, and a round that cannot fit is a frozen pot. V2
///      pays holders until gas runs low, banks a cursor, and resumes on the next
///      call. Normal rounds still finish in ONE transaction; only a pathological
///      holder set costs a second one. Neither outcome can freeze the pot.
///
/// @dev Trust model is unchanged: `poster` (the backend round-runner) supplies
///      the leaderboard shares, VRF supplies the winner. Everything else is
///      permissionless and re-validated on-chain, so Automation going away —
///      v2.1 sunsets 2026-07-31 — degrades liveness, never safety: any cron or
///      human can call the same public entrypoints.
contract DistributorV2 is VRFConsumerBaseV2Plus, Pausable, AutomationCompatibleInterface {
    // ---- errors ------------------------------------------------------------
    ...
    /// @dev Fields are ordered to pack the whole header into one slot.
    ///      `timeEnd` doubles as the round's opening timestamp — a round always
    ///      closes its window at the moment it is opened — so the shares
    ///      timeout needs no extra field.
    struct Round {
        uint8 status;
        bool hasRandom;
        uint8 vrfRetries;
        ...
        uint64 sharesAt; // when postShares landed; VRF timeout runs from here
        uint256 pot;
        uint256 random;
    ...
    ///         last round. Default 1785139200 = Monday 2026-07-27 08:00:00 UTC,
    ///         so with period = 1 week every round opens Monday 08:00 UTC.
    ...
    uint64 public scheduleAnchor = 1785139200;
    ...
    /// @notice Backend key allowed to post shares and force a cancel.
    ...
    uint64 public sharesTimeout = 6 hours; // poster silent -> cancel
    uint64 public vrfTimeout = 30 minutes; // VRF silent -> re-request
    uint8 public maxVrfRetries = 3; // then cancel and roll the pot forward
    ...
    uint16 public distributeBatchSize = 100;
    ...
    ///         100 of them would need ~6.6M — above Automation v2.1's per-upkeep
    ///         ceiling. Instead of guessing a batch size that fits every possible
    ///         holder set, the loop pays until gas runs low, banks the cursor, and
    ///         lets the next upkeep resume. Normal rounds finish in one call;
    ///         pathological ones take two. Neither can freeze the pot.
    ///         Must exceed one worst-case payout plus the round-closing tail.
    uint32 public gasFloor = 250000;
    ...
    /// `forced` distinguishes an admin override from a scheduled Monday round,
    /// so the two are auditable apart off-chain.
    ...
    event RoundDistributed(
        uint256 indexed roundId, address indexed winner, uint256 winnerAmount, uint256 distributedAmount, uint256 holderCount
    );
    ...
    constructor(address _vrfCoordinator, uint256 _vrfSubscriptionId, bytes32 _vrfKeyHash, address _poster)
        VRFConsumerBaseV2Plus(_vrfCoordinator)
    {
        ...
        poster = _poster;
        // First round covers everything from deployment onward.
    ...
    ///      underflow and brick the contract for good if the balance ever dipped
    ///      below what is owed.
    ...
    function pendingAction() public view returns (uint8 action, uint256 rid) {
        ...
        // opening an empty round that would just be cancelled 6 hours later.
    ...
    ///      and the period gate means it can fire at most once per period, so an
    ...
    /// @notice Open a round RIGHT NOW, ignoring the Monday-08:00 schedule. Admin
    ///         only (poster or owner) — this is the manual override for a missed
    ///         week, a one-off payout, or testing on a fresh deployment.
    /// @dev Does NOT shift the schedule. The next round is still the next Monday
    ///      08:00 slot, which may be only hours away — a forced round is an extra
    ///      payout inserted between slots, not a reset of the rhythm. The pot for
    ///      that next round is just the fees earned since, and a round with an
    ///      empty pot never opens, so back-to-back rounds are self-limiting.
    ...
    ///         holder in one call when the gas allows (the normal case, ~3.5M for
    ///         a full 100), otherwise stops at `gasFloor`, banks the cursor and
    ///         resumes on the next call. Call until the round reports status 2.
    ...
    /// @notice Move the scoring window by hand. Only needed to align the very
    ///         first round with the backend's epoch after a migration from V1 —
    ///         a live round would otherwise be paying out a window the backend
    ///         never scored.
