...
library PositionConfigIdLibrary {
    bytes32 constant MASK_UPPER_BIT =
    ...
    function getConfigId(
        PositionConfigId storage _configId
    ) internal view returns (bytes32 configId) {
    ...
    function hasSubscriber(
        PositionConfigId storage configId
    ) internal view returns (bool subscribed) {
