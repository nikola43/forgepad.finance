...
library VanityAddressLib {
    ...
    function betterThan(
        address first,
        address second
    ) internal pure returns (bool better) {
    ...
    function score(
        address addr
    ) internal pure returns (uint256 calculatedScore) {
        ...
        unchecked {
            ...
            uint256 leadingFourCount = getLeadingNibbleCount(
                addrBytes,
                leadingZeroCount,
                4
            );
    ...
    function getLeadingNibbleCount(
        bytes20 addrBytes,
        uint256 startIndex,
        uint8 comparison
    ...
    function getNibble(
        bytes20 input,
        uint256 nibbleIndex
    ) internal pure returns (uint8 currentNibble) {
