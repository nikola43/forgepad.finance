...
abstract contract Notifier is INotifier {
        ...
        onlyIfApproved(msg.sender, tokenId)
    {
        ...
        if (_subscriber != NO_SUBSCRIBER)
            revert AlreadySubscribed(tokenId, address(_subscriber));
        ...
        bool success = _call(
            newSubscriber,
            abi.encodeCall(ISubscriber.notifySubscribe, (tokenId, data))
        );

        if (!success) {
            newSubscriber.bubbleUpAndRevertWith(
                ISubscriber.notifySubscribe.selector,
                SubscriptionReverted.selector
            );
        }
    ...
    function _unsubscribe(uint256 tokenId) internal {
        ...
        if (address(_subscriber).code.length > 0) {
            ...
            if (gasleft() < unsubscribeGasLimit)
                GasLimitTooLow.selector.revertWith();
            try
                _subscriber.notifyUnsubscribe{gas: unsubscribeGasLimit}(tokenId)
            {} catch {}
        }
    ...
    function _removeSubscriberAndNotifyBurn(
        ...
        bool success = _call(
            _subscriber,
            abi.encodeCall(
                ISubscriber.notifyBurn,
                (tokenId, owner, info, liquidity, feesAccrued)
            )
        );
        ...
        if (!success) {
            _subscriber.bubbleUpAndRevertWith(
                ...
                BurnNotificationReverted.selector
            );
        }
    }
    ...
    function _notifyModifyLiquidity(
        ...
        bool success = _call(
            _subscriber,
            abi.encodeCall(
                ISubscriber.notifyModifyLiquidity,
                (tokenId, liquidityChange, feesAccrued)
            )
        );
        ...
        if (!success) {
            _subscriber.bubbleUpAndRevertWith(
                ...
                ModifyLiquidityNotificationReverted.selector
            );
        }
    }
