    ...
    NativeWrapper
{
    ...
    function _handleAction(
        ...
        if (action < Actions.SETTLE) {
            ...
            if (action == Actions.SETTLE_PAIR) {
                (Currency currency0, Currency currency1) = params
                    .decodeCurrencyPair();
                ...
                (Currency currency, uint256 amount, bool payerIsUser) = params
                ...
                (Currency currency, address recipient, uint256 amount) = params
                ...
                (Currency currency, uint256 amountMax) = params
                    .decodeCurrencyAndUint256();
                _clearOrTake(currency, amountMax);
                return;
            } else if (action == Actions.SWEEP) {
                (Currency currency, address to) = params
    ...
    function _increase(
        ...
        (PoolKey memory poolKey, PositionInfo info) = getPoolAndPositionInfo(
            tokenId
        );
    ...
    function _burn(
        ...
        uint256 liquidity = uint256(
            _getLiquidity(tokenId, poolKey, info.tickLower(), info.tickUpper())
        );
    ...
    function getPoolAndPositionInfo(
        uint256 tokenId
    ) public view returns (PoolKey memory poolKey, PositionInfo info) {
