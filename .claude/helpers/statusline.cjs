...
function getSwarmStatus() {
  ...
  // Check .claude-flow/swarm/swarm-state.json (nested swarms structure)
  ...
  if (swarmState && swarmState.swarms) {
    const swarmIds = Object.keys(swarmState.swarms);
    for (const id of swarmIds) {
      const s = swarmState.swarms[id];
      if (s.status === 'running') {
        const age = s.updatedAt ? now - new Date(s.updatedAt).getTime() : Infinity;
        if (age < staleThresholdMs) {
          totalAgents += (s.agents && s.agents.length) || s.agentCount || 0;
          maxAgents = Math.max(maxAgents, s.maxAgents || 0);
  ...
  // Check .swarm/state.json (flat structure from swarm start)
  const altStatePath = path.join(CWD, '.swarm', 'state.json');
  const altState = readJSON(altStatePath);
  if (altState && altState.status === 'initialized' && altState.agents) {
    const age = altState.startedAt ? now - new Date(altState.startedAt).getTime() : Infinity;
    if (age < staleThresholdMs) {
      totalAgents = Math.max(totalAgents, typeof altState.agents === 'number' ? altState.agents : (altState.agents.length || 0));
      coordinationActive = true;
    }
  }

  // Check swarm-activity.json metrics
  ...
  if (activityData && activityData.swarm) {
    ...
    if (age < staleThresholdMs) {
      totalAgents = Math.max(totalAgents, activityData.swarm.agent_count || 0);
      coordinationActive = coordinationActive || activityData.swarm.coordination_active || activityData.swarm.active || false;
    }
  }

  return { activeAgents: totalAgents, maxAgents, coordinationActive };
}
