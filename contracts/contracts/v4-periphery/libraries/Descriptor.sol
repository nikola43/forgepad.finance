...
library Descriptor {
    ...
    function constructTokenURI(
        ...
        string memory descriptionPartTwo = generateDescriptionPartTwo(
            ...
            escapeSpecialCharacters(params.baseCurrencySymbol),
            params.quoteCurrency == address(0)
                ? "Native"
                : addressToString(params.quoteCurrency),
            params.baseCurrency == address(0)
                ? "Native"
                : addressToString(params.baseCurrency),
    ...
    function escapeSpecialCharacters(
        ...
        if (specialCharCount > 0) {
            ...
            bytes memory escapedBytes = new bytes(
                symbolBytes.length + specialCharCount
            );
            uint256 index;
            for (uint8 i = 0; i < symbolBytes.length; i++) {
    ...
    function adjustForDecimalPrecision(
        ...
        uint256 difference = abs(
            int256(uint256(baseCurrencyDecimals)) -
                (int256(uint256(quoteCurrencyDecimals)))
        );
    ...
    function generateSVGImage(
        ...
        SVG.SVGParams memory svgParams = SVG.SVGParams({
            ...
            color0: currencyToColorHex(
                uint256(uint160(params.quoteCurrency)),
                136
            ),
            color1: currencyToColorHex(
                uint256(uint160(params.baseCurrency)),
                136
            ),
            color2: currencyToColorHex(
                uint256(uint160(params.quoteCurrency)),
                0
            ),
            color3: currencyToColorHex(
                uint256(uint160(params.baseCurrency)),
                0
            ),
            x1: scale(
