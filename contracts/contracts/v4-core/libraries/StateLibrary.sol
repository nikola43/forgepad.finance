...
library StateLibrary {
        ...
        returns (uint256 feeGrowthGlobal0, uint256 feeGrowthGlobal1)
    {
        ...
        bytes32 slot_feeGrowthGlobal0X128 = bytes32(
            uint256(stateSlot) + FEE_GROWTH_GLOBAL0_OFFSET
        );
    ...
    function getTickBitmap(
        ...
        bytes32 tickBitmapMapping = bytes32(
            uint256(stateSlot) + TICK_BITMAP_OFFSET
        );

        // slot id of the mapping key: `pools[poolId].tickBitmap[tick]
        bytes32 slot = keccak256(
            abi.encodePacked(int256(tick), tickBitmapMapping)
        );
    ...
    function _getPositionInfoSlot(
        ...
        bytes32 positionMapping = bytes32(
            uint256(stateSlot) + POSITIONS_OFFSET
        );
