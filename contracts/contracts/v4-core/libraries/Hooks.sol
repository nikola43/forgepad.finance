...
library Hooks {
    ...
    function isValidHookAddress(
        ...
        return
            address(self) == address(0)
                ? !fee.isDynamicFee()
                : (uint160(address(self)) & ALL_HOOK_MASK > 0 ||
                    fee.isDynamicFee());
    }
    ...
    function callHook(
        ...
        assembly ("memory-safe") {
            ...
            mstore(
                0x40,
                add(result, and(add(returndatasize(), 0x3f), not(0x1f)))
            )
        ...
        if (
            result.length < 32 || result.parseSelector() != data.parseSelector()
        ) {
        ...
        if (self.hasPermission(AFTER_INITIALIZE_FLAG)) {
            self.callHook(
                abi.encodeCall(
                    ...
                    (msg.sender, key, sqrtPriceX96, tick)
                )
            );
        }
    }
    ...
    function afterModifyLiquidity(
        ...
        if (msg.sender == address(self))
            return (delta, BalanceDeltaLibrary.ZERO_DELTA);
    ...
    function afterSwap(
        ...
        int128 hookDeltaUnspecified = beforeSwapHookReturn
            .getUnspecifiedDelta();
    ...
    function beforeDonate(
        ...
        if (self.hasPermission(BEFORE_DONATE_FLAG)) {
            self.callHook(
                abi.encodeCall(
                    IHooks.beforeDonate,
                    (msg.sender, key, amount0, amount1, hookData)
                )
            );
        }
    }
